...
func (r *Replicator) replicate(addr string, leave chan struct{}) {
	...
	if err != nil {
		r.logErr(err, "failed to dial", addr)
	}
	defer cc.Close()
	...
	if err != nil {
		r.logErr(err, "failed to consume", addr)
		return
	}
	...
	go func() {
		for {
			...
			if err != nil {
				r.logErr(err, "failed to receive", addr)
				return
			}
			records <- recv.Record
		}
	...
	for {
		...
		case record := <-records:
			...
			if err != nil {
				r.logErr(err, "failed to produce", addr)
				return
			}
		}
	}
...
func (r *Replicator) logErr(err error, msg string, addr string) {
	r.logger.Error(
		msg,
		zap.Error(err),
