...
func (p *Picker) Pick(info balancer.PickInfo) (balancer.PickResult, error) {
	...
	if strings.Contains(info.FullMethodName, "Produce") || len(p.follower) == 0 {
...
func (p *Picker) nextFollower() balancer.SubConn {
	...
	len := uint64(len(p.follower))
	idx := int(cur % len)
	return p.follower[idx]
}
